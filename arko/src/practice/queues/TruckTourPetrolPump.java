...
public class TruckTourPetrolPump {
	...
	public static void main(String[] args) {
		...
		while (sc.hasNext()) {
			...
			int distToNext[] = new int[n];
			int petrolAvailable = 0;
			int intial = 0 ;
			for (int i = 0; i < n; i++) {
				petrol[i] = sc.nextInt();
				distToNext[i] = sc.nextInt();
				petrolAvailable += (petrol[i]);
				if(petrolAvailable<distToNext[i]){
					intial = i+1;
					petrolAvailable=0;
				}
				else
					petrolAvailable -= distToNext[i];
			}
